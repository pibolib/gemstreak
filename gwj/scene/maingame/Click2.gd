...
func _process(delta):
	if canrotate and Input.is_action_just_released("mouse_left") and !get_owner().gameover and !get_owner().pause:
		var save = get_parent().get_parent().currenttri[0]
		get_parent().get_parent().currenttri[0] = get_parent().get_parent().currenttri[2]
		get_parent().get_parent().currenttri[2] = get_parent().get_parent().currenttri[1]
	...
	if canrotate and Input.is_action_pressed("mouse_left") and !get_owner().gameover and !get_owner().pause:
		get_parent().region_rect.position = Vector2(96,112)
	else:
		get_parent().region_rect.position = Vector2(96,96)
